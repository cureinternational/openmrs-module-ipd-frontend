  ...
  slots.forEach((slot) => {
    if (
      ...
      if (slot.serviceType === asNeededPlaceholderConceptName)
        time = slot.medicationAdministration?.administeredDateTime / 1000;
      else time = slot.startTime;
      administrationInfo.push({
        kind: slot.administrationSummary.status,
        time: enable24HourTime
          ? formatDate(time * 1000, timeFormatFor24Hr)
          : formatDate(time * 1000, timeFormatFor12Hr),
      });
    }
  });
