...
export const Header = ({ timeframeLimitInHours, navHourEpoch }) => {
  const { ipdConfig, taskFilterType = TASK_FILTER_HEADER.ALL, setTaskFilterType } = useContext(CareViewContext);
  const { enable24HourTime = {} } = ipdConfig;
  const intl = useIntl();
  const taskFilterTypes = [TASK_FILTER_HEADER.ALL, TASK_FILTER_HEADER.NEW, TASK_FILTER_HEADER.PENDING];
    ...
    <tr className="patient-row-container">
      ...
      >
        <ContentSwitcher
          ...
          selectedIndex={selectedTaskFilterIndex >= 0 ? selectedTaskFilterIndex : 0}
          data-testid="task-filter-switcher"
        >
          <Switch
            name={TASK_FILTER_HEADER.ALL}
            text={intl.formatMessage({ id: TASK_FILTER_HEADER.ALL, defaultMessage: "All" })}
            data-testid="filter-tab-all"
          />
          <Switch
            name={TASK_FILTER_HEADER.NEW}
            text={intl.formatMessage({ id: TASK_FILTER_HEADER.NEW, defaultMessage: "New" })}
            data-testid="filter-tab-new"
          />
          <Switch
            name={TASK_FILTER_HEADER.PENDING}
            text={intl.formatMessage({ id: TASK_FILTER_HEADER.PENDING, defaultMessage: "Pending" })}
